...
**The note.** Your focus carries the plan map from prettyfocussed.com (`web/assets/pf.css` › THE MAP, ported as `.pf-map`/`.pf-blk`): three blocks on the three atmosphere plates in a glass frame, area proportional to weight, the number large with its mono unit, her words under it, a glass tag on top with the word for the year. The blocks are the goals that get this month (else the first three open quarter goals): lane as the overline, progress as the number, what is left as the weight, the goal as the words; with no goals yet the three blocks are the word for the year, the month's focus and the first obstacle. It sits sticky to the right of the chapters from 980px and first on narrower screens, so the reminder comes before the questions. Hover moves the sharp plane, as on the site.

**Two lanes on the note** (25 September, on Laurence's ask for a place for her personal brand): the note carries one map per lane, "Professional focus" and "Personal focus", three blocks each, the goals chosen for the month first; an empty slot is a fainter plate that says "Room for one more." and opens the goals, so the three-per-lane limit reads as room, not as a wall. The lanes are named Professional and Personal throughout (`LANE_LABEL`), where they were Business and Life.

**The breath.** One circle (`BreathRing`): a Haze-to-Sage disc inside a soft halo, scaling between 0.72 and 1 over eight seconds on a symmetric curve, the word Inhale or Exhale at its centre, a mono count when there is a target. Ambient motion on the brand clock, and the only loop in the product. Reduced motion holds the disc still and keeps the words. Two places: on the first open of the day a sheet, "Take a breath.", three cycles then it closes itself, "Not now" is one tap and the day is marked (`contextStore.lastBreath`); and in the focus timer, twenty minutes into a running focus the circle appears under the clock with "20 minutes in. Breathe with it.", and a text button "Breathe" calls it any time.
...
- **The week, ported from prettyfocussed.com** (25 September, later the same day): Laurence asked for the exact UI of the marketing site, so the calendar now uses its components verbatim from `web/assets/pf.css` › THE WEEK in `/Users/laurence/Downloads/pretty-focussed`: the week card on the Air-to-Shell gradient with noise (`.pf-sched`), seven glass day cards with mono day, display number and plate dots (`.pf-dayc`; the chosen day is Ink, today an Ink ring, a free day fainter), the white day panel (`.pf-dayplan`) and the slot rows on atmosphere plates (`.pf-slot`: plate 1 High Air for professional, plate 3 Last Light for personal, plate 2 Overcast for unassigned) with the time in display size, the area as overline, the title, the length in mono. The app's secondary button is now the site's glass pill. Motion follows the site and Emil Kowalski's rules: the day strip never animates (tapped many times), the day panel's content resolves in 220ms from a 6px blur, the page header and the week card resolve on arrival on the brand clock with 80ms steps, every pressable scales to 0.97, and a slot in use rises sharp while its neighbours go soft. The task list uses the same slot rows in day panels per group. The section tabs are the app's segmented pill and every string on this page is English.
- **Your priorities · the week strip and the task card** (25 September, on Laurence's reference): the calendar is one week as seven cards (mono day, big number, up to four dots for what sits on the day: Focus for professional, Sage for personal, Stone for unassigned, half for done; the chosen day is Ink with Bone text, today carries an Ink ring), chevrons a week at a time with "This week / Next week / Last week" and the date span. Under it the chosen day as a list of task cards: a circle check, the start time in mono at 26px (or TIME, tap to set), the sphere or priority as a caps label over the title, the duration at right (tap cycles 15 min to 3 h), and small icons for day, note and remove that are always visible so touch works. The same card serves the Taken list, grouped Overdue, Today, Planned, Someday. Quick add is one line plus sphere chips, a time, a duration and, in the list, a date. On the phone the card drops its right cluster to a second line and titles may wrap. Todos gained `time` and `minutes`.
- **Bottom nav on the phone**: the header holds the fixed nav and must sit above main (`.pf-head2 { z-index:403 }`); without it the page painted over the nav and taps fell through.
- **Your priorities** (25 September) mounts the Laurence OS Productivity page (to-dos with spheres and priority, the day calendar, notes linked to tasks, the focus timer) as the second segment, in the same brand pass.
