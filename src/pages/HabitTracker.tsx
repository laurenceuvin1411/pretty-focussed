...
const CAT_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  persoonlijk:   { label: 'Personal',      color: '#C4935A', bg: 'rgba(196,136,78,0.08)' },
  professioneel: { label: 'Professional',  color: '#6DB889', bg: 'rgba(109,184,137,0.08)' },
  lifestyle:     { label: 'Personal',      color: '#C4935A', bg: 'rgba(196,136,78,0.08)' },
  physical:      { label: 'Physical',      color: '#C4935A', bg: 'rgba(196,136,78,0.08)' },
  mental:        { label: 'Mental',        color: '#7AACCF', bg: 'rgba(122,172,207,0.08)' },
  professional:  { label: 'Professional',  color: '#6DB889', bg: 'rgba(109,184,137,0.08)' },
  financial:     { label: 'Financial',     color: '#D4A96A', bg: 'rgba(212,169,106,0.08)' },
  relationships: { label: 'Relationships', color: '#7AACCF', bg: 'rgba(122,172,207,0.08)' },
}
...
function AddHabitModal({ onClose, editHabit }: { onClose: () => void; editHabit?: Habit }) {
  ...
  async function save() {
    ...
    const habitData = {
      ...
      icon: '', color: CAT_CONFIG[category]?.color ?? '#6DB889',
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(6px)' }}>
      <div style={{ width: '100%', maxWidth: 520, background: 'var(--color-card)', borderRadius: 20, border: '1px solid var(--color-border)', padding: '28px 28px 32px', margin: '0 16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <h3 style={{ fontSize: 17, fontWeight: 700, color: 'var(--color-ink)' }}>{editHabit ? 'Edit habit' : 'New habit'}</h3>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <input
            ...
            style={{ width: '100%', padding: '14px 16px', borderRadius: 12, border: '1px solid var(--color-border)', background: 'var(--color-surface)', color: 'var(--color-ink)', fontSize: 15, outline: 'none', boxSizing: 'border-box' }}
          />

          <div>
            <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-subtle)', marginBottom: 8 }}>Category</p>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {ALL_CATS.map(c => (
                <button key={c} onClick={() => setCategory(c)}
                  style={{ padding: '8px 16px', borderRadius: 99, fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 150ms',
          ...
          <div>
            <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-subtle)', marginBottom: 8 }}>Frequency</p>
            <div style={{ display: 'flex', gap: 8 }}>
              {(['daily', 'weekdays', 'custom'] as const).map(f => (
                <button key={f} onClick={() => setFreq(f)}
                  style={{ padding: '8px 14px', borderRadius: 99, fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 150ms',
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 12 }}>
                {dayLabels.map((d, i) => (
                  <button key={i} onClick={() => setCustomDays(prev => prev.includes(i) ? prev.filter(x => x !== i) : [...prev, i])}
                    style={{ width: 36, height: 36, borderRadius: 16, fontSize: 11, fontWeight: 700, cursor: 'pointer', transition: 'all 150ms',
          ...
          <button onClick={save} disabled={saving || !name.trim()}
            style={{ marginTop: 8, padding: '15px', borderRadius: 12, background: 'var(--color-accent)', border: 'none', color: 'var(--color-bg)', fontSize: 14, fontWeight: 700, cursor: 'pointer', opacity: saving || !name.trim() ? 0.5 : 1 }}>
    ...
    <div style={{ borderRadius: 14, border: '1px solid var(--color-border)', background: 'var(--color-card)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '18px 20px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12, gap: 12 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color, padding: '2px 8px', borderRadius: 99, background: `${color}15` }}>
                {areaLabel[goal.area] ?? goal.area}
              </span>
            </div>
            <p style={{ fontSize: 15, fontWeight: 700, color: 'var(--color-ink)', lineHeight: 1.3 }}>{goal.title}</p>
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 700, color, minWidth: 36, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{pct}%</span>
        </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            {editing ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <button onClick={saveProgress} style={{ padding: '5px 10px', borderRadius: 16, background: color, border: 'none', color: 'var(--color-bg)', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>OK</button>
              ...
              <button onClick={() => setEditing(true)} style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-subtle)', fontSize: 12, padding: 0 }}>
                <span style={{ fontWeight: 600, color: 'var(--color-ink)' }}>{goal.currentNumber}</span>
            ...
            {goal.milestones.map((m, i) => {
              ...
              return (
                <div key={i} style={{ flex: 1, textAlign: 'center', opacity: unlocked ? 1 : 0.35 }}>
                  ...
                  <p style={{ fontSize: 9, color: unlocked ? color : 'var(--color-subtle)', fontWeight: 600, lineHeight: 1.3 }}>{m.label}</p>
                </div>
              )
            })}
          </div>
    ...
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 0 }}
      ...
      <button onClick={onToggle}
        ...
        <div style={{
          ...
          border: done ? `2px solid ${cfg.color}` : '2px solid rgba(124,127,132,0.28)',
          transition: 'all 200ms cubic-bezier(.16,1,.3,1)',
          boxShadow: done ? `0 0 10px ${cfg.color}50` : 'none',
        }}>
          {done && <Check size={13} color="#0A0805" strokeWidth={3} />}
        </div>
        ...
        {streak > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '3px 8px', borderRadius: 99, background: 'rgba(245,158,11,0.12)', border: '1px solid rgba(245,158,11,0.2)' }}>
            <Flame size={10} color="#B8956A" />
            <span style={{ fontSize: 10, fontWeight: 700, color: '#B8956A' }}>{streak}</span>
          </div>
        )}

        {/* Month % */}
        <span style={{ fontSize: 11, fontWeight: 600, color: monthPct >= 75 ? '#6DB889' : monthPct >= 50 ? '#D4A96A' : 'var(--color-subtle)', minWidth: 28, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>
          {monthPct}%
        </span>
      </button>
  ...
  return (
    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {hovered && (
            <button
              ...
              style={{ width: 18, height: 18, borderRadius: 4, border: '1px solid var(--color-border)', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-subtle)', flexShrink: 0, padding: 0 }}
            >
              <Trash2 size={10} />
            </button>
          )}
      ...
      {days.map(d => {
        ...
        const past = isPast(d)
        const bg = d === todayDay ? 'rgba(212,169,106,0.06)' : 'transparent'
        return (
          <td key={d} style={{ padding: '6px 2px', textAlign: 'center', background: bg }}>
            {past ? (
              <button onClick={() => toggleHabit(habit.id, format(new Date(year, month, d), 'yyyy-MM-dd'))}
                style={{ width: 20, height: 20, borderRadius: 5, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 120ms',
                  ...(log?.completed
                    ? { background: cfg.color, border: `1.5px solid ${cfg.color}`, boxShadow: `0 0 6px ${cfg.color}60` }
                    : { background: 'transparent', border: '1.5px solid rgba(124,127,132,0.28)' }) }}>
                {log?.completed && <Check size={10} color="#0A0805" strokeWidth={3} />}
              </button>
            ) : (
              <div style={{ width: 20, height: 20, borderRadius: 5, margin: '0 auto', background: 'transparent', border: '1.5px solid rgba(124,127,132,0.16)' }} />
            )}
          </td>
        )
      })}
      <td style={{ padding: '12px 8px', textAlign: 'center', fontWeight: 700, color: scoreColor, fontSize: 11 }}>{pct}%</td>
    </tr>
  )
}
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--color-subtle)', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button onClick={() => setShowAICoach(true)}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 14px', borderRadius: 16, border: '1px solid var(--color-border)', background: 'transparent', color: 'var(--color-subtle)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
            <Sparkles size={12} /> Coach
          </button>
          <button onClick={() => setShowAddHabit(true)}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 14px', borderRadius: 16, border: 'none', background: 'var(--color-accent)', color: 'var(--color-bg)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
            <Plus size={13} /> Habit
          </button>
        </div>
      </div>
      ...
      {/* ── Tabs ───────────────────────────────────────────────── */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 24, padding: '4px', background: 'rgba(255,255,255,0.04)', borderRadius: 12, border: '1px solid var(--color-border)', width: 'fit-content' }}>
        {([['today', 'Today'], ['month', 'Month'], ['goals', 'Goals']] as const).map(([t, label]) => (
          <button key={t} onClick={() => setTab(t)}
            style={{ padding: '8px 20px', borderRadius: 9, fontSize: 13, fontWeight: tab === t ? 700 : 500, cursor: 'pointer', transition: 'all 150ms',
      ...
      {tab === 'today' && (
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
            <button onClick={() => goDay(-1)} style={{ width: 32, height: 32, borderRadius: 16, border: '1px solid var(--color-border)', background: 'var(--color-card)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-muted)' }}>
              <ChevronLeft size={15} />
            </button>
            <div style={{ flex: 1, textAlign: 'center' }}>
              ...
              {!isToday && (
                <button onClick={() => setSelectedDateStr(todayStr)} style={{ marginLeft: 10, fontSize: 11, fontWeight: 600, color: 'var(--color-accent)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                  Back to today
                </button>
              )}
            </div>
            <button onClick={() => goDay(1)} disabled={isToday} style={{ width: 32, height: 32, borderRadius: 16, border: '1px solid var(--color-border)', background: 'var(--color-card)', cursor: isToday ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: isToday ? 'var(--color-subtle)' : 'var(--color-muted)', opacity: isToday ? 0.35 : 1 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 28 }}>
            {[
              { label: 'Today', value: `${todayDone}/${todayTotal}`, sub: 'habits', color: '#6DB889', icon: <Check size={12} /> },
              { label: 'Score', value: `${todayScore}%`, sub: 'day score', color: todayScore >= 80 ? '#6DB889' : todayScore >= 50 ? '#D4A96A' : '#C4935A', icon: <Star size={12} /> },
              { label: 'Streak', value: `${perfectDays}`, sub: 'perfect days', color: '#B8956A', icon: <Flame size={12} /> },
              { label: 'Month', value: `${monthScore}%`, sub: 'month score', color: monthScore >= 75 ? '#6DB889' : '#D4A96A', icon: <TrendingUp size={12} /> },
            ].map(s => (
              <div key={s.label} style={{ borderRadius: 12, border: '1px solid var(--color-border)', background: 'var(--color-card)', padding: '16px 18px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5, color: 'var(--color-subtle)', marginBottom: 8 }}>{s.icon}</div>
                <p style={{ fontSize: 24, fontWeight: 800, color: s.color, letterSpacing: '-0.04em', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{s.value}</p>
          ...
          {/* Week heatmap */}
          <div style={{ borderRadius: 12, border: '1px solid var(--color-border)', background: 'var(--color-card)', padding: '16px 20px', marginBottom: 24 }}>
            <p style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--color-subtle)', marginBottom: 12 }}>Week</p>
            <div style={{ display: 'flex', gap: 6 }}>
              {weekDays.map(d => {
                ...
                const col = pct >= 1 ? '#6DB889' : pct >= 0.5 ? '#D4A96A' : '#C4935A'
                return (
                  <div key={dStr}
                    ...
                    <div style={{
                      ...
                      {!isFut && activeHabits.length > 0 && <span style={{ fontSize: 10, fontWeight: 700, color: pct >= 0.5 ? col : 'var(--color-subtle)' }}>{done}/{activeHabits.length}</span>}
                    </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {presentCats.map(cat => {
              ...
              return (
                <div key={cat} style={{ borderRadius: 14, border: '1px solid var(--color-border)', background: 'var(--color-card)', overflow: 'hidden', marginBottom: 4 }}>
                  {/* Category header */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 20px 10px', borderBottom: '1px solid var(--color-border)' }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: cfg.color, flex: 1 }}>
                      {cfg.label}
                    </span>
                    <span style={{ fontSize: 11, fontWeight: 600, color: catDone === catHabits.length ? cfg.color : 'var(--color-subtle)' }}>
          ...
          {activeHabits.length === 0 && (
            <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--color-subtle)' }}>
              ...
              <button onClick={() => setShowAddHabit(true)} style={{ marginTop: 16, padding: '10px 20px', borderRadius: 16, background: 'var(--color-accent)', border: 'none', color: 'var(--color-bg)', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                Add habit
              </button>
            </div>
          )}
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 24 }}>
            {[
              ...
              { label: 'Done this month', value: monthlyDone, color: '#6DB889', icon: <Check size={12} /> },
              { label: 'Perfect days', value: perfectDays, color: '#B8956A', icon: <Flame size={12} /> },
              { label: 'Month score', value: `${monthScore}%`, color: monthScore >= 75 ? '#6DB889' : monthScore >= 50 ? '#D4A96A' : '#C4935A', icon: <Star size={12} /> },
            ].map(s => (
              <div key={s.label} style={{ borderRadius: 12, border: '1px solid var(--color-border)', background: 'var(--color-card)', padding: '16px 18px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5, color: 'var(--color-subtle)', marginBottom: 8 }}>{s.icon}</div>
                <p style={{ fontSize: 24, fontWeight: 800, color: s.color, letterSpacing: '-0.04em', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{s.value}</p>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
            <button onClick={() => setViewDate(new Date(year, month - 1, 1))}
              style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: '1px solid var(--color-border)', borderRadius: 9, cursor: 'pointer', color: 'var(--color-muted)' }}>
              <ChevronLeft size={14} />
            </button>
            <span style={{ padding: '0 8px', fontSize: 14, fontWeight: 600, color: 'var(--color-ink)', textTransform: 'capitalize' }}>
          ...
          {/* Grid */}
          <div style={{ borderRadius: 14, border: '1px solid var(--color-border)', background: 'var(--color-card)', overflow: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 800, fontSize: 12 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
                  <th style={{ textAlign: 'left', padding: '14px 20px', fontWeight: 600, fontSize: 10, letterSpacing: '0.14em', color: 'var(--color-subtle)', width: 200 }}>HABIT</th>
                  {days.map(d => (
                    <th key={d} style={{ padding: '14px 0', textAlign: 'center', width: 26, fontWeight: d === todayDay ? 700 : 400, fontSize: 10, color: d === todayDay ? 'var(--color-accent)' : 'var(--color-subtle)', background: d === todayDay ? 'rgba(212,169,106,0.08)' : 'transparent' }}>{d}</th>
                  ))}
                ...
                {presentCats.map(cat => {
                  ...
                  return [
                    <tr key={`cat-${cat}`} style={{ background: cfg.bg, borderBottom: '1px solid var(--color-border)' }}>
                      <td style={{ padding: '8px 20px', fontSize: 9, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: cfg.color, borderLeft: `3px solid ${cfg.color}` }} colSpan={daysInMonth + 2}>
                        {cfg.label}
                      </td>
                    </tr>,
                    ...catHabits.map(habit => {
                      ...
                      const scoreColor = pct >= 75 ? '#6DB889' : pct >= 50 ? '#D4A96A' : '#C4935A'
                      return (
                        <MonthHabitRow
                          key={habit.id}
                          habit={habit}
      ...
      {tab === 'goals' && (
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <div>
              <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-ink)' }}>{goals.filter(g => g.status === 'active').length} active goals</p>
          ...
          {['business', 'health', 'lifestyle', 'finance', 'relationships'].map(area => {
            ...
            const areaColor: Record<string, string> = {
              business: '#6DB889', health: '#C4935A', lifestyle: '#7AACCF',
              finance: '#D4A96A', relationships: '#7AACCF',
            }
            return (
              <div key={area} style={{ marginBottom: 28 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: areaColor[area] }} />
                  <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: areaColor[area] }}>{areaLabel[area]}</p>
                </div>
